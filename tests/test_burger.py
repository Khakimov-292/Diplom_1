...
class TestBurger:
    ...
    def test_add_ingredient(self, burger_fixture, ingredient_filling_mock):
        burger_fixture.add_ingredient(ingredient_filling_mock)
        assert burger_fixture.ingredients[0].name == INGREDIENT_FILLING_NAME

    def test_remove_ingredient(self, burger_fixture, ingredient_filling_mock):
        burger_fixture.add_ingredient(ingredient_filling_mock)
        burger_fixture.remove_ingredient(0)
        assert len(burger_fixture.ingredients) == 0

    def test_move_ingredient(self, burger_fixture, ingredient_sauce_mock, ingredient_filling_mock):
        burger_fixture.add_ingredient(ingredient_filling_mock)
    ...
    def test_get_burger_price(self, burger_fixture, bun_mock, ingredient_filling_mock):
        burger_fixture.set_buns(bun_mock)
        burger_fixture.add_ingredient(ingredient_filling_mock)
        assert burger_fixture.get_price()

    @pytest.mark.parametrize('number_of_ingredients', [0, 1, 2, 3, 4])
    def test_get_burger_price_is_correct(self, number_of_ingredients, burger_fixture, bun_mock, ingredient_sauce_mock,
                                         ingredient_filling_mock):
        burger_fixture.set_buns(bun_mock)
    ...
    def test_get_receipt(self, burger_fixture, bun_mock, ingredient_filling_mock,
